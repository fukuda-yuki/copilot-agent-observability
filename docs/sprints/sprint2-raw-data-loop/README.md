...
この README の目的、検討候補、成功条件候補は M1 の仕様化入力であり、`../../requirements.md` と `../../spec.md` に反映されるまで確定仕様ではない。
...
Langfuse 依存の PoC から、raw telemetry / normalized dataset / improvement loop を中心にした構成へ移行できるかを検討する。

Langfuse を raw data の source of truth ではなく、人間が trace を確認する dashboard / trace viewer として再位置づけできるかを検討する。
...
改善ループを Langfuse API ではなく、raw store または normalized dataset を主入力にできるかを検討する。
...
- `diagnose`: 人間が分類した diagnosis record を検証する。trace からの自動診断を Sprint2 MVP に含めるかは M1 で確定する。
...
- Langfuse を source of truth ではなく dashboard / trace viewer として扱えるかを検討する。
- raw store に保存されたデータを改善ループの主入力にできるかを検討する。
