...
- [ ] content / credential / identity-bearing data が output に混入しないことを確認する
...
3. raw store 入力と raw JSON file 入力の分岐を command boundary に閉じ込める。
4. `aggregate-measurements` の既存 tests が regress しないことを確認する。
