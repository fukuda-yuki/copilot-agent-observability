...
Sprint3 では、trace content を含む観測データから deterministic rule により診断候補、改善候補、人間レビューへ接続する readiness record を生成する後続基盤を扱う。
...
Sprint3 では、明示的に仕様化した範囲に限り、診断候補生成、改善候補生成、人間レビューへ接続する readiness record 生成を後続スコープとして扱う。
自動採用判断と自動改善実装は Sprint4 以降の候補とする。
...
Sprint3 では、改善候補の deterministic 生成と、人間レビューへ接続する readiness record 生成を扱う。
改善案の自動採用、実 repository file の自動修正、patch / diff の生成、commit / push / pull request 作成は、Sprint3 では扱わず、Sprint4 以降で安全境界を定義してから扱う。
...
Sprint3 では、trace からの deterministic な自動診断候補生成に加え、content-aware evidence、改善候補生成、人間レビューへ接続する readiness record 生成の基盤を扱う。
...
実 repository 修正を伴う自動改善実装は Sprint4 以降の候補として扱う。
