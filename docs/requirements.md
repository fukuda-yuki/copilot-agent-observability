...
Sprint4 M2 の dashboard dataset contract では、TTFT、estimated cost、stuck / long-running 判定、time bucket を以下の方針で扱う。
TTFT は直接属性がある場合のみその値を使い、ない場合は first generation event / chunk / span start から導出する fallback 指標として扱う。
導出できない場合は null とし、取得元を dataset に記録する。
estimated cost は token 数と model 別 unit price table に基づく観測用の概算値であり、Copilot の実課金額として扱わない。
unit price が不明な場合は null とし、cost source に不明理由を記録する。
stuck session、long-running trace、long-running turn、long-running tool は dashboard dataset generation parameter の閾値で判定し、M2 contract の既定値を初期値とする。
time bucket の既定粒度は day とし、prototype 検証用に hour / week を許可する。
...
Outcome Linkage の着手優先度は Sprint4 M5 handoff で Tier 分けし、M2 dashboard dataset contract では placeholder / future candidate としてのみ扱った。
Tier 0 は Sprint4 初期 dashboard に含めない範囲であり、外部 API ingestion、production GitHub / Notion / HR integration、identity mapping、org usage / ROI dashboard を含む。
Tier 1 は sanitized / manual reference による future planning candidate であり、PR / issue / CI / review placeholder や手動で sanitized した outcome reference を含むが、外部 API 実装は約束しない。
Tier 2 は product / security decision 後のみ実装可能な範囲であり、shared dashboard、team / department 表示、実 GitHub / Notion ingestion は retention、access control、masking、利用者周知を先に仕様化するまで実装しない。
Tier 3 は明示的な非目的であり、個人別生産性評価、勤務監視、ランキング、HR data correlation、経営向け利用状況 dashboard は Sprint4 と初期 Sprint5 dashboard work では実装しない。
...
* GitHub / Notion / issue / PR 等の outcome linkage は将来候補として境界と Tier が定義されている
