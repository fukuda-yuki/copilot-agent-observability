...
Local Ingestion Monitor は、`raw-local-receiver` の telemetry をローカルで確認するための単一 ASP.NET Core プロセス（loopback-only）であり、Sprint8 で実装した。Sprint7 の Config CLI receiver（`127.0.0.1:4319`）と並存し、別 loopback port（既定 `127.0.0.1:4320`、Collector の `4317`/`4318` と Sprint7 CLI receiver の `4319` を回避）で動作する。OTLP HTTP/protobuf を受信して SQLite raw store に永続化し、sanitized monitor projection、ローカル UI、health endpoint を提供する。Sprint9 で受信済み OTel テレメトリから per-span の sanitized projection（`monitor_spans`）を追加し、agent-execution view としてツール / MCP 呼び出し名、成否、sub-agent のモデル / トークン、turn 単位トークンを表示する。raw body と PII は既定で表示する（server-rendered、inert text）。`--sanitized-only` フラグで metadata-only モードを復元できる。Sprint10 では trace-detail page を 2 section 構成にする：上段は JS が sanitized spans JSON を取得して描画する tab 群（Summary / Timeline / Flow Chart / Cache）、下段は従来どおり Razor が server-render する raw OTLP preview（D023 境界維持）。`--sanitized-only` 時は上段の sanitized tab shell は引き続き表示し、下段 raw preview と full raw link だけを除外する。Flow Chart は vendored Cytoscape.js + dagre（`wwwroot/vendor/`、CDN 不使用）で span 階層を DAG 描画し、Timeline は errors-only filter と tokens / time sort、Cache は trace 内 cache-hit rate / token breakdown を示す。typography は vendored Noto Sans JP / Noto Sans Mono。これらは新エンドポイント・新 API field を追加せず、既存 `GET /api/monitor/traces/{traceId}/spans` 上の client-side presentation に限る（build step なし）。詳細は [specifications/layers/telemetry-ingestion.md](specifications/layers/telemetry-ingestion.md) と [specifications/security-data-boundaries.md](specifications/security-data-boundaries.md) を正本とする。

Sprint11 の GitHub Copilot app Canvas adapter は、Local Ingestion Monitor の thin adapter である。Project-scoped extension は `.github/extensions/otel-monitor-canvas/extension.mjs` に置き、`joinSession({ canvases: [createCanvas({...})] })` の Copilot SDK pattern を使う。`open()` は extension-owned loopback URL または diagnostic を返し、agent-callable actions は既存 sanitized monitor APIs の bounded DTO だけを返す。Canvas-safe posture は Local Monitor を `--sanitized-only` で起動することとし、adapter は raw-bearing default TraceDetail surface を開かない。Canvas adapter は monitor UI を再実装せず、telemetry input、SQLite schema、normalized dataset、candidate records、dashboard contracts、`/api/monitor/*` response shape、raw boundary を変更しない。
...
- Local Ingestion Monitor raw-bearing surfaces（既定表示）: trace-detail page の下段 raw section（bounded raw preview inline + full raw record link）および `GET /traces/{rawRecordId}/raw`（server-rendered HTML）。raw-bearing surface では same-origin 強制（cross-site は `403`）、`Cache-Control: no-store`。`--sanitized-only` 起動時は trace-detail page の sanitized tab shell は `200` で残し、raw section / full raw link を除外し、`GET /traces/{rawRecordId}/raw` は `404`、PII は除外。JSON raw API は提供しない。
- Local Ingestion Monitor run interface: loopback port（既定 `http://127.0.0.1:4320`）、`--port` / `--url`、`--sanitized-only`（metadata-only モード。raw section / full raw route を除外し PII を除外。sanitized TraceDetail tabs は利用可能）、リクエスト本文サイズ上限 `--max-request-body-bytes`（既定 `31457280` bytes = 30 MiB、env `CAO_MONITOR_MAX_REQUEST_BODY_BYTES`）。`POST /v1/traces` は本文が上限を超えると `413` / `request_too_large` を返し raw を書かない。
...
- Sprint10 design views（Flow Chart / Cache Explorer / timeline filter-sort / themed UI）は **public interface を追加しない**。新エンドポイント・新 API field・新 query parameter はなく、既存 `GET /api/monitor/traces/{traceId}/spans` の sanitized 出力を client-side で描画するのみ。raw-bearing endpoint は追加しない。
- GitHub Copilot app Canvas adapter public surface: project-scoped `.github/extensions/otel-monitor-canvas/extension.mjs`、ES modules、`joinSession({ canvases: [createCanvas({...})] })`、loopback-only `open()` result、`onClose()` cleanup、`session.log()` diagnostics。`/create-canvas` は `.github/skills/create-canvas/SKILL.md` の skill workflow であり、`.github/prompts/*.prompt.md` として実装しない。Scaffold は `extensions_manage({ operation: "scaffold", kind: "canvas", name: "otel-monitor-canvas", location: "project" })` を使い、tool unavailable 時は blocker を記録して停止する。Hand-written fallback は明示承認がある場合だけ許可する。Documented Canvas actions are `monitor_health()`, `list_recent_traces({ limit, status?, model? })`, `get_trace_summary({ traceId })`, `get_trace_span_tree({ traceId })`, and `get_cache_summary({ traceId })`; action outputs are bounded sanitized DTOs, with `list_recent_traces.limit` constrained to `1..50` for Canvas output.
