...
- Local Ingestion Monitor raw-bearing routes（既定表示）: trace-detail page（agent-execution view、raw body inline）および `GET /traces/{rawRecordId}/raw`（server-rendered HTML）。raw-bearing route set の全 route で same-origin 強制（cross-site は `403`）、`Cache-Control: no-store`。`--sanitized-only` 起動時は raw-bearing route は `404`、PII は除外。JSON raw API は提供しない。
