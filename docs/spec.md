...
Sprint3: Deterministic Trace Diagnosis Candidate Pipeline は要件定義段階である。
Sprint3 では、Sprint2.5 の結果を踏まえ、trace からの deterministic な自動診断候補生成、content-aware evidence extraction、改善候補生成、人間レビューへ接続する readiness record 生成の基盤を扱う。
自動採用判断、実 repository 修正を伴う自動改善実装、patch / diff 生成は Sprint3 の既定スコープには含めず、Sprint4 以降の候補として安全境界を定義してから扱う。
...
Sprint3 では、明示的に仕様化した範囲に限り、人間レビューへ接続する readiness record 生成を扱う。
自動採用判断と自動改善実装への接続は Sprint4 以降の候補として扱う。
...
Sprint3 の readiness record 生成と Sprint4 以降の自動採用判断 / 自動改善実装の検討は、M11-M22 の研究計測フェーズとは別スコープで扱う。
...
Sprint3 では、この loop の前段として deterministic candidate pipeline と human-review readiness record を扱う。
自動採用判断と自動改善実装への接続は Sprint4 以降の候補として扱う。
...
Sprint3 では、人間レビューへ接続する readiness record を扱う。
自動採用判断、実 repository 修正を伴う自動改善実装、自動 commit、自動 push、自動 pull request は Sprint3 の既定スコープ外とし、Sprint4 以降で安全境界を定義してから扱う。
...
Sprint3 で仕様化する readiness record は、Sprint3 の安全境界内で人間レビュー接続に留まる限り `F-SCOPE` や `AP-AUTO-DECIDE` とは扱わない。
一方で、Sprint3 の範囲を超えた自動採用、repository 修正、patch / diff 生成、commit / push / pull request、自動勝敗決定を行う提案は、引き続き `F-SCOPE` または `AP-AUTO-DECIDE` の対象とする。
...
Sprint3 では、Sprint2 raw data loop と Sprint2.5 maintainability の成果を前提に、trace content を含む観測データから deterministic rule により診断候補、改善候補、人間レビューへ接続する readiness record を生成する基盤を定義する。
Sprint3 は要件定義中であり、command contract、candidate schema、content evidence schema、readiness decision schema は sprint-local material で確定してから本書に反映する。

現時点で本書に確定する Sprint3 の対象は以下に限定する。

- trace-driven diagnosis candidate、improvement proposal candidate、human-review readiness decision record を検討する。
...
Sprint3 では人間レビューへ接続する readiness record 生成を扱う。
改善案の自動採用判断、実 repository 修正を伴う自動改善実装、patch / diff 生成は Sprint4 以降の候補として扱う。
