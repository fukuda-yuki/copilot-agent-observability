...
Sprint4: Agent Workflow Observability Dashboard は完了済みである。
Sprint4 では、既存の OTel 収集、Langfuse trace viewer、raw telemetry store、normalized measurement、Sprint3 candidate pipeline を前提に、Agent workflow 改善判断のための dashboard 要件を定義した。
...
Sprint4 M2 dashboard dataset には、raw prompt / response / tool arguments / tool results、credential、secret、Base64 header、実 user identity を既定で保存しない。
Sprint5 static dashboard では、この historical Sprint4 方針を更新し、`user_id` と `user_email` に限って dashboard dataset と static dashboard の表示・filter 対象に含める。
...
Outcome Linkage の着手優先度は Sprint4 M5 handoff で Tier 分けした。
...
#### Sprint5 static dashboard planning update

Sprint5 planning では、Enterprise Grafana の導入・運用負荷を避けるため、GitHub Pages 向け static HTML dashboard を第一候補に変更する。
この変更は Sprint4 M5 時点の historical handoff を置き換えるものではなく、Sprint5 開始前の planning decision として扱う。

Sprint5 static dashboard は Grafana 代替の常設 dashboard として利用する。
GitHub Pages は private repository と明示的な access control を前提にした公開先候補である。
Dashboard には実データ由来の集計値、参照 ID、分類属性を publish してよい。
ただし、raw prompt / response / system prompt / tool arguments / tool results、source code fragments、file contents、credential、secret、Base64 authorization header、sensitive bundle content、sensitive bundle local path は表示しない。

Sprint5 static dashboard では `user.id` と `user.email` を表示・filter 対象に含める。
`user.email` は実値をそのまま表示する。
将来的に email と display name の mapping を入力し、email 表示と display name 表示を切り替えられるようにする。

GitHub Actions は毎日実行し、raw store / normalized dataset から dashboard dataset を再生成する。
Actions は生成した HTML と JSON dataset を GitHub Pages に publish する。
Publish layout は `/latest/` と `/YYYY-MM-DD/` の日次 snapshot とする。
日次 snapshot は保持し、自動削除しない。
HTML と JSON dataset は repository または Pages branch に保存してよい。

初期 view set は Sprint4 の view set を踏襲する。
...
初期 client-side interaction は filter、sort、search とする。
初期 filter 軸は date、user、client、experiment、variant、status とする。

Grafana JSON dashboard は将来候補または fallback として残す。
Static report は非インタラクティブな review packet として扱えるが、Sprint5 の主経路は GitHub Pages 向け static HTML dashboard とする。

#### Sprint5 static dashboard implementation contract

Sprint5 の初期実装では、`generate-dashboard-dataset --json` が出力する dashboard dataset JSON を static HTML dashboard generator の入力とする。

Generator command は以下とする。

```text
config-cli generate-static-dashboard <dashboard-dataset.json> --out-dir <output-dir> [--snapshot-date <YYYY-MM-DD>] [--title <title>]
```

`generate-static-dashboard` は `index.html` と `dashboard-data.json` を出力する。
`index.html` は static file とし、server-side API、runtime service、network dependency を要求しない。
`dashboard-data.json` は input dashboard dataset の sanitized copy とし、raw prompt / response / system prompt / tool arguments / tool results、source code fragment、credential、secret、Base64 authorization header、sensitive bundle content、sensitive bundle local path を含めない。
`user.id` と `user.email` は raw OTLP Resource Attributes から取得できる場合に、dashboard dataset 上では `user_id` と `user_email` として保持する。
`user_id` と `user_email` は static dashboard の表示および filter / search 対象に含める。

GitHub Actions の初期 publish workflow は `schedule` と `workflow_dispatch` で起動する。
Actions は repository secret、Langfuse credential、OTLP authorization header を要求しない。
Actions の初期入力候補は `artifacts/dashboard-input/` 配下の normalized measurement、raw OTLP、candidate outputs とする。
`artifacts/dashboard-input/measurements.json` が存在する場合は、それを使って dashboard dataset を再生成する。
存在しない場合は repository の synthetic fixture から preview dashboard dataset を生成し、workflow と Pages artifact の構文検証用 dashboard として publish する。

Publish artifact layout は以下とする。
