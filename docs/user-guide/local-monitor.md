...
| `--sanitized-only` | off | metadata-only モード。TraceDetail の sanitized tab は表示し、raw section / full raw route と PII を除外する。画面共有や health-check 時に使用 |
...
出ず、`GET /traces/{rawRecordId}/raw` は `404` です。画面共有や health-check 時に便利です。
