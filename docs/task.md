...
| Local Raw Receiver Monitor | 実装中（M1 完了 / Phase 0 spec promotion 済み） | [docs/sprints/sprint8-local-raw-receiver-monitor/](sprints/sprint8-local-raw-receiver-monitor/) で ConfigCli から共有 `Telemetry` / `Persistence.Sqlite` project を抽出した（M1）。OTLP/ingest/normalize/SQLite 処理を共有化し、T4 重複解消・NU1903 解消・291 tests green を維持。replan（[requirements-and-replan.md](sprints/sprint8-local-raw-receiver-monitor/requirements-and-replan.md)）と adversarial review を経て DR1–DR6 / DD1–DD6 を確定し、Phase 0 で正本（requirements / spec / specifications / decisions D020）へ promotion 済み。M2〜M6（ASP.NET Core monitor host、bounded-channel queue/worker、sanitized projection + opt-in raw/PII view（`--enable-raw-view`、既定 off、loopback-only）、Web UI/SSE、health/readiness（飽和時 非2xx）、live validation）は未着手 |
