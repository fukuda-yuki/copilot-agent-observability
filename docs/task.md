...
| Area | 状態 | 概要 |
| --- | --- | --- |
| Monitor Visual Implementation | **計画中 / Sprint12 M5-M6** | Sprint12 の [Design Shape](sprints/sprint12-monitor-design-shape/) を同じ Sprint12 内で実装へ反映する。M1-M4 で `DESIGN.md`、CSS token architecture、Index / Traces / TraceDetail / Ingestions / Diagnostics の page layout spec を整理済み。M5 で Local Monitor の `monitor.css`、共通レイアウト、各 Razor 表示を整え、M6 で build、Playwright Chromium bootstrap、full test、主要ページ screenshot / browser smoke、raw 境界、`--sanitized-only` 表示を確認する。既存の telemetry 入力、SQLite schema、API field、raw / sanitized 境界、Canvas adapter contract は変更しない。 |
| Monitor Design Shape | **Sprint12 M1-M4 完了** | [docs/sprints/sprint12-monitor-design-shape/](sprints/sprint12-monitor-design-shape/) で、Sprint10/Sprint11 後の Local Monitor visual foundation を Shape / documentation alignment として整理した。Sprint10 の実装前工程ではなく、既存の VS Code Dark+ テーマ方針、Noto Sans JP / Noto Sans Mono vendored typography、OKLCH color tokens、8px spacing、CSS token architecture、Index / Traces / TraceDetail / Ingestions / Diagnostics のページレイアウト仕様を履歴として再位置付けした。M5-M6 で実装と検証へ進む。 |
| GitHub Copilot Canvas Adapter PoC | **完了** | [docs/sprints/sprint11-github-copilot-canvas-adapter-poc/](sprints/sprint11-github-copilot-canvas-adapter-poc/) で、Local Ingestion Monitor を GitHub Copilot app Canvas から参照する thin adapter を追加した。M1 で `/create-canvas` skill workflow、project-scoped scaffold-first 方針、`--sanitized-only` Canvas-safe posture、bounded sanitized action contracts、raw/PII 禁止境界を canonical specs に反映した。M2 で project-scoped extension scaffold、Canvas id `otel-monitor`、`open()`（loopback-only 検証、`/health/ready` 健全性チェック、非健全時診断ページ）、`onClose()` を実装した。M3/M4 で `monitor_health`、`list_recent_traces`、`get_trace_summary`、`get_trace_span_tree`、`get_cache_summary` を `.github/extensions/otel-monitor-canvas/extension.mjs` に追加し、既存 sanitized `/api/monitor/*` と `/health/ready` だけを読む bounded DTO action とした。M5（UI trigger）を実装済み（D029）: `open()` は拡張所有の loopback ヘルパーページ（per-launch token）を返し、trace ドロップダウン・focus セレクタ・「Analyze selected trace with Copilot」ボタンを提供、ボタン押下で `session.send()` により sanitized-only 指示を Copilot へ投入する。M6（検証・ドキュメント）で build 0/0・test 555 passing（300 ConfigCli + 255 LocalMonitor）を確認し、Canvas runtime tools が利用可能な環境で live Canvas validation を実施した。`extensions_reload`/`extensions_manage`/`list_canvas_capabilities`/`open_canvas`/`invoke_canvas_action` で全 5 action の起動・呼び出しを検証、schema 拒否・sanitized DTO・loopback/token 保護・`--sanitized-only` 姿勢（raw route 404・TraceDetail sanitized tabs）の全ネガティブチェック通過。user guide に Canvas adapter セクションを追加。M6 evidence: [sprints/sprint11-github-copilot-canvas-adapter-poc/milestones/M6-validation-and-docs/live-validation.md](sprints/sprint11-github-copilot-canvas-adapter-poc/milestones/M6-validation-and-docs/live-validation.md)。 |
