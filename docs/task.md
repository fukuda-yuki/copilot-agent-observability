...
| Sprint3: Deterministic Trace Diagnosis Candidate Pipeline | 要件定義中 | [docs/sprints/sprint3-trace-diagnosis/](sprints/sprint3-trace-diagnosis/) で trace content を含む deterministic 診断候補生成、改善候補生成、人間レビューへ接続する readiness record の基盤を定義する |
...
- Sprint3 は要件定義中であり、Sprint2 MVP と Sprint2.5 で除外した trace からの自動診断候補生成を、deterministic rule、content-aware evidence、改善候補生成、人間レビューへ接続する readiness record まで拡張して扱う。
- Sprint3 M1 は `candidate-schema-and-command-boundary` とし、candidate 専用 command / schema、readiness decision 専用 schema、sensitive output 保存先、synthetic fixture 方針を sprint-local に確定した。
- Sprint3 M2-M5 は、deterministic rule / evidence contract、diagnosis candidate 実装、improvement / readiness 実装、M24-M27 human-review pipeline 接続を扱う。
...
- Sprint3 では、trace から failure category / anti-pattern 候補を deterministic に自動抽出し、改善候補と人間レビューへ接続する readiness record に接続する。
