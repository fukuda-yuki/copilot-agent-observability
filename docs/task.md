...
| Sprint5: Static HTML Observability Dashboard | 完了 | [docs/sprints/sprint5-static-dashboard/](sprints/sprint5-static-dashboard/) で GitHub Pages 向け static HTML dashboard を Grafana 代替の常設 dashboard 第一候補として扱った |
...
- Sprint5 M6 `review and handoff` は完了した。Sprint5 の実装、運用境界、残課題を review し、GitHub Pages access control、初回 live workflow 実行、snapshot growth monitoring、email / display name mapping、external outcome linkage を後続へ分離した。
...
- Sprint5 M6 では、generated daily snapshots は `gh-pages` branch と Pages artifact に置き、`main` には commit しない境界を再確認した。実データ由来 aggregate を publish する前に、repository / organization の GitHub Pages access control と初回 workflow 実行結果を環境側で確認する。
