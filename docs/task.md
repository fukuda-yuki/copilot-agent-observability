...
| Local Raw Receiver Monitor | 実装中（M1 完了） | [docs/sprints/sprint8-local-raw-receiver-monitor/](sprints/sprint8-local-raw-receiver-monitor/) で ConfigCli から共有 `Telemetry` / `Persistence.Sqlite` project を抽出した（M1）。OTLP/ingest/normalize/SQLite 処理を共有化し、T4 重複解消・NU1903 解消・291 tests green を維持。M2〜M6（ASP.NET Core monitor host、bounded-channel queue/worker、sanitized monitor projection、Web UI/SSE、live validation）は未着手 |
