...
| Area | 状態 | 概要 |
| --- | --- | --- |
| GitHub Copilot Canvas Adapter PoC | **計画中 / M5 planned** | [docs/sprints/sprint11-github-copilot-canvas-adapter-poc/](sprints/sprint11-github-copilot-canvas-adapter-poc/) で、Local Ingestion Monitor を GitHub Copilot app Canvas から参照する thin adapter を追加する。M1 で `/create-canvas` skill workflow、project-scoped scaffold-first 方針、`--sanitized-only` Canvas-safe posture、bounded sanitized action contracts、raw/PII 禁止境界を canonical specs に反映した。M2 で project-scoped extension scaffold、Canvas id `otel-monitor`、`open()`（loopback-only 検証、`/health/ready` 健全性チェック、非健全時診断ページ）、`onClose()` を実装した。M3/M4 で `monitor_health`、`list_recent_traces`、`get_trace_summary`、`get_trace_span_tree`、`get_cache_summary` を `.github/extensions/otel-monitor-canvas/extension.mjs` に追加し、既存 sanitized `/api/monitor/*` と `/health/ready` だけを読む bounded DTO action とした。あわせて full-suite で再現した `IngestionWriterWorker` shutdown drain race を修正し、accepted queue items を落とさないよう writer reader loop 起動を `StartAsync` の完了条件にした。構文・静的検査、M3/M4 contract tests、build、Playwright install、`dotnet test CopilotAgentObservability.slnx`（554 passing）は通過。Canvas runtime validation tools はこの Codex surface では未提供。M5（UI trigger）へ移行。 |
