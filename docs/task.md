...
| Area | 状態 | 概要 |
| --- | --- | --- |
| Local Monitor Copilot Raw Analysis | **実装中 / BYOK live validation 部分完了** | [docs/sprints/sprint13-local-monitor-copilot-raw-analysis/](sprints/sprint13-local-monitor-copilot-raw-analysis/) で、Local Monitor TraceDetail から raw analysis run を開始し、.NET GitHub Copilot SDK analysis service へ dispatch する local surface を追加した。raw analysis result は local runtime data、GitHub 向けは repository-safe summary のみ。`--sanitized-only` では raw analysis UI / routes を無効化する。自動テストは追加済み。2026-06-30 の live validation で loopback 起動、synthetic trace ingestion、TraceDetail、analysis run 作成、repository-safe summary の raw marker / PII 非混入を確認した。初回 SDK 実行は OS error 5 で失敗したが、BYOK provider と明示的な SDK `BaseDirectory` により analysis run が `succeeded` になった。Evidence: [sprints/sprint13-local-monitor-copilot-raw-analysis/milestones/M6-validation-and-closeout/live-validation.md](sprints/sprint13-local-monitor-copilot-raw-analysis/milestones/M6-validation-and-closeout/live-validation.md)。 |
...
- **技術負債（2026-06-30 SSOT 監査より、欠陥ではなく保守性項目として延期）**
  ...
  - Canvas extension contract test が文字列部分一致検査中心で、構文エラーや helper-server token / request shaping 回帰を検出できない（F8）。`extension.mjs` 大改修前に JS レベルの実行可能 smoke coverage を追加する。
