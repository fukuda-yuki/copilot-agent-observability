...
Phase 1 では、Docker Desktop 上の Langfuse self-host Docker Compose を既定の実行基盤とし、VS Code GitHub Copilot Chat / GitHub Copilot CLI から Langfuse に OTLP HTTP で直接送信します。ローカルでは Langfuse UI 到達、初期ユーザー / organization / project / API key の作成まで確認済みです。
...
Config CLI には、Phase 1 の Langfuse 向け設定サンプルを出力する `langfuse-vscode-settings`、`langfuse-vscode-env`、`langfuse-copilot-cli-env` があります。
