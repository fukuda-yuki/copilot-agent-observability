...
送信時の認証は、Langfuse project の public key / secret key を Base64 化した Basic Auth を使います。詳細な環境変数と実行手順は `docs/spec.md`、全体の作業状態は `docs/task.md`、milestone 別の詳細は `docs/milestones/` 配下を参照してください。

実行手順と検証項目の正は `docs/spec.md` と該当 milestone の `task.md` です。
...
- `docs/spec.md`: 確定仕様・実装判断の主 source of truth
- `docs/task.md`: milestone 一覧と全体状態の index
- `docs/milestones/`: milestone ごとの task、plan、questions、review、notes
- `docs/knowledge/`: milestone を跨いで再利用する調査結果、判断理由、確認済み事項
- `docs/archive/`: 完了済み記録と過去レビュー
